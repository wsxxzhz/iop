...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,com.girlsfrontline.demo";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("com.girlsfrontline.demo", 1, 9, "抽卡机", "1.0.0", 1, "IOP_technology", "抽卡机", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("com.girlsfrontline.demo", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("com.girlsfrontline.demo", AppData.CQLog);	
